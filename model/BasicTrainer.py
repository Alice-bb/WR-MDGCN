...
#         total_loss = 0
#         epoch_time = time.time()
#         #开始迭代训练数据集。enumerate(self.train_loader)将self.train_loader中的数据进行迭代，
#         # batch_idx是批次的索引，(data, target)是从数据加载器中获取的一个批次数据和目标。
...
#
#     def train(self):#训练过程包括两个阶段，每个阶段有多个训练周期（epochs）
...
#                                     "Training stops. total time: {:.2f} s".format(self.args.early_stop_patience, time.time() - train_time))
#                     break
#             # save the best state
#             if best_state == True:
...
#         self.test(self.model, self.args, self.test_loader, self.scaler, self.logger)#测试最佳模型
#
...
#
#     @staticmethod
#     #用于计算在调度采样（scheduled sampling）中使用的采样概率。
...
class Trainer(object):
    ...
    def test(model, args, data_loader, scaler, logger, path=None):
        ...
        if args.real_value:
            ...
            y_pred = torch.cat(y_pred, dim=0)
            y_true = torch.cat(y_true, dim=0)
        np.save('./{}_true1.npy'.format(args.dataset), y_true.cpu().numpy())
        np.save('./{}_pred1.npy'.format(args.dataset), y_pred.cpu().numpy())
        print(f"Pred shape: {y_pred.shape}, True shape: {y_true.shape}")
        for t in range(y_true.shape[1]):
            mae, rmse, mape, _, pcc = All_Metrics(y_pred[:, t, ...], y_true[:, t, ...],
                                                args.mae_thresh, args.mape_thresh)
